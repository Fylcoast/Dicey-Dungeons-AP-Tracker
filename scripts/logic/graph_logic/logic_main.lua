...
dicey_dungeons_location = {}
dicey_dungeons_location.__index = dicey_dungeons_location
...
function dicey_dungeons_location.new(name)
    local self = setmetatable({}, dicey_dungeons_location)
    if name then
...
function dicey_dungeons_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = dicey_dungeons_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function dicey_dungeons_location:connect_two_ways(exit, rule)
...
function dicey_dungeons_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function dicey_dungeons_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function dicey_dungeons_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function dicey_dungeons_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function dicey_dungeons_location:accessibility()
...
-- 
function dicey_dungeons_location:discover(accessibility, keys)
...
entry_point = dicey_dungeons_location.new("entry_point")
